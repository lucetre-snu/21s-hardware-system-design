...
module mv_multiplier #(
    ...
    reg [BITWIDTH-1:0] gb2[0:VECTOR_SIZE-1];
    
    reg [BITWIDTH-1:0] ain[0:VECTOR_SIZE-1];
    reg [BITWIDTH-1:0] bin;
    reg valid = 0;
    
    wire [BITWIDTH-1:0]  out[0:VECTOR_SIZE-1];
    wire [BITWIDTH-1:0] dout[0:VECTOR_SIZE-1];
    wire dvalid;
    ...
    always @(*)
        case (present_state)
            ...
            S_LOAD: if (cnt_load == MATRIX_SIZE+VECTOR_SIZE-1)  next_state = S_CALC; else next_state = present_state;
    ...
    always @(dvalid or present_state)
        if (present_state == S_CALC) begin
            if (dvalid) begin
                ...
                for (row = 0; row < VECTOR_SIZE; row = row+1) begin
                    ain[row] <= gb1[row*VECTOR_SIZE + cnt_calc];
                end
                bin <= gb2[cnt_calc];
                valid <= 1;
            end
    ...
    generate for (i = 0; i < 2**L_RAM_SIZE; i = i+1) begin: VECTOR
