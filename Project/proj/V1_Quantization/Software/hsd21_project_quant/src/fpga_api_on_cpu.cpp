...
const float* FPGA::blockMM(Compute* comp)
{
  ...
  if(comp->quantized)
  {
    ...
    int *offset = reinterpret_cast<int*>(output_M);
    int *a1 = reinterpret_cast<int*>(qout_);
    int *a2 = reinterpret_cast<int*>(output_);
    ...
    dequantize1(qout_M, out, m1_size_, 0, weight_scale*act_scale);
  }
  else{
    for(int i = 0; i < v_size_; ++i)
    {
...
const float *FPGA::blockMV(Compute* comp)
{
  ...
  if(comp->quantized)
  {
    ...
    dequantize1(qout_, out, v_size_, 0, act_scale*weight_scale);
  }
  else
  {
    for (int i = 0; i < m_size_; ++i)
